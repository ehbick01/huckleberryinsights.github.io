...
# Plot count of age
age_hist <- euth_train_tbl %>% 
  ggplot(aes(x = age)) + 
  geom_histogram_interactive(fill = "#404040",
                             aes(
                               tooltip = paste0(
                                 "<br><b> Age: </b>",
                                 age %>% comma(0)
...
ggiraph(code = print(age_hist), 
        width = 1, 
        height = 6, 
        hover_css = "cursor:pointer;fill:#ff8500;stroke:#ff8500;")

# We can do a better job of transforming the data
log_age_hist <- euth_train_tbl %>% 
...
ggiraph(code = print(log_age_hist), 
        width = 1, 
        height = 6, 
        hover_css = "cursor:pointer;fill:#ff8500;stroke:#ff8500;")
...
los_hist <- euth_train_tbl %>% 
  ...
  geom_histogram_interactive(fill = "#404040",
                             ...
                             aes(
                                tooltip = paste0(
                               "<br><b> Length of Stay: </b>",
                               length_of_stay %>% comma(0)
                               ),
                             data_id = paste0(
                               "<br><b> Length of Stay: </b>",
                               length_of_stay %>% comma(0)
...
ggiraph(code = print(los_hist), 
        width = 1, 
        height = 6, 
        hover_css = "cursor:pointer;fill:#ff8500;stroke:#ff8500;")

# Plot count of length_of_stay
los_binned_hist <- euth_train_tbl %>% 
  filter(length_of_stay <= 365) %>%
  group_by(length_of_stay) %>% 
  summarise(total = n()) %>% 
  ggplot(aes(x = length_of_stay)) + 
  geom_histogram_interactive(bins = 5,
                             fill = "#404040",
                             colour = "#ffffff",
                             aes(
                                tooltip = paste0(
...
ggiraph(code = print(los_binned_hist), 
        width = 1, 
        height = 6, 
        hover_css = "cursor:pointer;fill:#ff8500;stroke:#ff8500;")
